...
//  MEBNumberKeyboard
//
//  Created by 搞得赢 on 2017/12/09.
//  Copyright © 2017年 Meituan. All rights reserved.
//

#import "MEBNumberKeyboard.h"

typedef NS_ENUM(NSUInteger, MEBNumberKeyboardButton) {
    MEBNumberKeyboardButtonNumberMin,
    MEBNumberKeyboardButtonNumberMax = MEBNumberKeyboardButtonNumberMin + 10, // Ten digits.
    MEBNumberKeyboardButtonBackspace,
    MEBNumberKeyboardButtonDone,
    MEBNumberKeyboardButtonSpecial,
    MEBNumberKeyboardButtonDecimalPoint,
    MEBNumberKeyboardButtonMinus,
    MEBNumberKeyboardButtonNone = NSNotFound,
};

@interface MEBNumberKeyboard () <UIInputViewAudioFeedback>
...
@interface _MEBNumberKeyboardButton : UIButton

+ (_MEBNumberKeyboardButton *)keyboardButtonWithStyle:(MEBNumberKeyboardButtonStyle)style;

// The style of the keyboard button.
@property (assign, nonatomic) MEBNumberKeyboardButtonStyle style;
...
@implementation MEBNumberKeyboard

static const NSInteger MEBNumberKeyboardRows = 4;
static const CGFloat MEBNumberKeyboardRowHeight = 55.0f;
static const CGFloat MEBNumberKeyboardPadBorder = 7.0f;
static const CGFloat MEBNumberKeyboardPadSpacing = 8.0f;
...
- (void)_commonInit
{
    ...
    // Add default action.
   // UIImage *dismissImage = [self.class _keyboardImageNamed:@"MEBNumberKeyboardDismissKey.png"];
    ...
    [self setReturnKeyButtonStyle:MEBNumberKeyboardButtonStyleDone];
...
- (void)_configureButtonsForCurrentStyle
{
    ...
    const NSInteger numberMin = MEBNumberKeyboardButtonNumberMin;
    const NSInteger numberMax = MEBNumberKeyboardButtonNumberMax;
    ...
    for (MEBNumberKeyboardButton key = numberMin; key < numberMax; key++) {
        UIButton *button = [_MEBNumberKeyboardButton keyboardButtonWithStyle:MEBNumberKeyboardButtonStyleWhite];
    ...
    UIImage *backspaceImage = [self.class _keyboardImageNamed:@"MEBNumberKeyboardDeleteKey.png"];
    
    UIButton *backspaceButton = [_MEBNumberKeyboardButton keyboardButtonWithStyle:MEBNumberKeyboardButtonStyleGray];
    ...
    [(_MEBNumberKeyboardButton *)backspaceButton addTarget:self action:@selector(_backspaceRepeat:) forContinuousPressWithTimeInterval:0.15f];
    
    [buttonDictionary setObject:backspaceButton forKey:@(MEBNumberKeyboardButtonBackspace)];
    
    UIButton *specialButton = [_MEBNumberKeyboardButton keyboardButtonWithStyle:MEBNumberKeyboardButtonStyleGray];
    
    [buttonDictionary setObject:specialButton forKey:@(MEBNumberKeyboardButtonSpecial)];
    
    UIButton *doneButton = [_MEBNumberKeyboardButton keyboardButtonWithStyle:MEBNumberKeyboardButtonStyleDone];
    [doneButton.titleLabel setFont:doneButtonFont];
    [doneButton setTitle:UIKitLocalizedString(@"Done") forState:UIControlStateNormal];
    
    [buttonDictionary setObject:doneButton forKey:@(MEBNumberKeyboardButtonDone)];
    
    UIButton *decimalPointButton = [_MEBNumberKeyboardButton keyboardButtonWithStyle:MEBNumberKeyboardButtonStyleWhite];
    ...
    [buttonDictionary setObject:decimalPointButton forKey:@(MEBNumberKeyboardButtonDecimalPoint)];
    
    UIButton *minusButton = [_MEBNumberKeyboardButton keyboardButtonWithStyle:MEBNumberKeyboardButtonStyleWhite];
    [minusButton setTitle:@"-" forState:UIControlStateNormal];
    
    [buttonDictionary setObject:minusButton forKey:@(MEBNumberKeyboardButtonMinus)];
...
{
    __block MEBNumberKeyboardButton keyboardButton = MEBNumberKeyboardButtonNone;
    
    [self.buttonDictionary enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        MEBNumberKeyboardButton k = [key unsignedIntegerValue];
        if (button == obj) {
            keyboardButton = k;
            *stop = YES;
        }
    ...
    if (keyboardButton == MEBNumberKeyboardButtonNone) {
        return;
    }
    
    // Get first responder.
    id <UIKeyInput> keyInput = self.keyInput;
    id <MEBNumberKeyboardDelegate> delegate = self.delegate;
    ...
    // Handle backspace.
    else if (keyboardButton == MEBNumberKeyboardButtonBackspace) {
    ...
    // Handle done.
    else if (keyboardButton == MEBNumberKeyboardButtonDone) {
    ...
    // Handle special key.
    else if (keyboardButton == MEBNumberKeyboardButtonSpecial) {
    ...
    // Handle .
    else if (keyboardButton == MEBNumberKeyboardButtonDecimalPoint) {
    ...
    // Handle -
    else if (keyboardButton == MEBNumberKeyboardButtonMinus) {
        NSString *minusText = [button titleForState:UIControlStateNormal];
        if ([delegate respondsToSelector:@selector(numberKeyboard:shouldInsertText:)]) {
            BOOL shouldInsert = [delegate numberKeyboard:self shouldInsertText:minusText];
            if (!shouldInsert) {
                return;
            }
        }
        
        [keyInput insertText:minusText];
    }
}
...
- (void)configureSpecialKeyWithImage:(UIImage *)image actionHandler:(dispatch_block_t)handler
{
    ...
    UIButton *button = self.buttonDictionary[@(MEBNumberKeyboardButtonSpecial)];
...
- (void)setReturnKeyTitle:(NSString *)title
{
    if (![title isEqualToString:self.returnKeyTitle]) {
        UIButton *button = self.buttonDictionary[@(MEBNumberKeyboardButtonDone)];
        if (button) {
...
- (void)setReturnKeyButtonStyle:(MEBNumberKeyboardButtonStyle)style
{
    if (style != _returnKeyButtonStyle) {
        _returnKeyButtonStyle = style;
        
        _MEBNumberKeyboardButton *button = self.buttonDictionary[@(MEBNumberKeyboardButtonDone)];
        if (button) {
            button.style = style;
        }
    }
...
- (void)setPreferredStyle:(MEBNumberKeyboardStyle)style
{
...
NS_INLINE CGRect MMButtonRectMake(CGRect rect, CGRect contentRect, BOOL usesRoundedCorners){
    ...
    if (usesRoundedCorners) {
        CGFloat inset = MEBNumberKeyboardPadSpacing / 2.0f;
...
- (void)layoutSubviews
{
    ...
    if ([UITraitCollection class]) {
        ...
        if (hasMargins || isIdiomPad) {
            ...
            const BOOL prefersPlainButtons = (self.preferredStyle == MEBNumberKeyboardStylePlainButtons);
            const BOOL prefersRoundedButtons = (self.preferredStyle == MEBNumberKeyboardStyleRoundedButtons);
    ...
    const CGFloat spacing = (usesRoundedButtons) ? MEBNumberKeyboardPadBorder : 0.0f;
    ...
    const CGFloat rowHeight = CGRectGetHeight(contentRect) / MEBNumberKeyboardRows;
    ...
    // Layout special key.
    UIButton *specialKey = buttonDictionary[@(MEBNumberKeyboardButtonMinus)];
    if (specialKey) {
        CGRect rect = (CGRect){ .size = numberSize };
        rect.origin.y = numberSize.height * 3;
    ...
    UIButton *decimalPointKey = buttonDictionary[@(MEBNumberKeyboardButtonDecimalPoint)];
    if (decimalPointKey) {
        CGRect rect = (CGRect){ .size = numberSize };
        rect.origin.y = numberSize.height * 3;
    ...
    const int utilityButtonKeys[2] = { MEBNumberKeyboardButtonBackspace, MEBNumberKeyboardButtonDone };
    ...
    for (NSInteger idx = 0; idx < sizeof(utilityButtonKeys) / sizeof(int); idx++) {
        MEBNumberKeyboardButton key = utilityButtonKeys[idx];
    ...
    for (_MEBNumberKeyboardButton *button in buttonDictionary.allValues) {
...
- (CGSize)sizeThatFits:(CGSize)size
{
    ...
    const CGFloat spacing = (interfaceIdiom == UIUserInterfaceIdiomPad) ? MEBNumberKeyboardPadBorder : 0.0f;
    
    size.height = MEBNumberKeyboardRowHeight * MEBNumberKeyboardRows + (spacing * 2.0f);
...
@interface _MEBNumberKeyboardButton ()
...
@implementation _MEBNumberKeyboardButton

+ (_MEBNumberKeyboardButton *)keyboardButtonWithStyle:(MEBNumberKeyboardButtonStyle)style
{
    _MEBNumberKeyboardButton *button = [self buttonWithType:UIButtonTypeCustom];
    button.style = style;
...
- (void)setStyle:(MEBNumberKeyboardButtonStyle)style
{
    if (style != _style) {
        _style = style;
...
{
    const UIUserInterfaceIdiom interfaceIdiom = UI_USER_INTERFACE_IDIOM();
    const MEBNumberKeyboardButtonStyle style = self.style;
    
    UIColor *fillColor = nil;
    UIColor *highlightedFillColor = nil;
    if (style == MEBNumberKeyboardButtonStyleWhite) {
        fillColor = [UIColor whiteColor];
        highlightedFillColor = [UIColor colorWithRed:0.82f green:0.837f blue:0.863f alpha:1];
    } else if (style == MEBNumberKeyboardButtonStyleGray) {
    ...
    } else if (style == MEBNumberKeyboardButtonStyleDone) {
    ...
    if (style == MEBNumberKeyboardButtonStyleDone) {
