...
@implementation NSString (MEBTextField)
...
{
    if ([self pp_is:PPTextFieldStringTypeNumber] || [self pp_is:PPTextFieldStringTypeLetter] || [self pp_is:PPTextFieldStringTypeChinese] || [self pp_is:PPTextFieldStringTypeNineGridChinese]) {
        return NO;
    }
    return YES;
}
...
-(BOOL)matchRegularWith:(PPTextFieldStringType)type
{
    ...
    switch (type) {
        ...
        case PPTextFieldStringTypeChinese:     //汉字
            regularStr = @"^[\u4e00-\u9fa5]{0,}$";
            break;
        case PPTextFieldStringTypeNineGridChinese:
            regularStr = @"^[➋-➒]*$";
        default:
            break;
    }
